...
parser.add_argument('--test', type=str, default='data/penn/',
...
args.tied = True
args.tied_lm = True
suffix = f"_{args.data.split('/')[-1]}_{args.test.split('/')[-2]}"

print("Makedirs")
os.makedirs(f"./data/decode_{args.save}{suffix}/data")
os.makedirs(f"./data/decode_{args.save}{suffix}/data_out")
os.makedirs(f"./data/decode_{args.save}{suffix}/msgs")
os.makedirs(f"./data/decode_{args.save}{suffix}/msgs_out")
os.makedirs(f"./data/decode_{args.save}{suffix}/msgs_out_sig")
print("Makeddirs")
...
def load_other_test(path):
    def _split(line):
        result = line.split()
        if corpus.tokens is not None:
            result = [x if x in corpus.tokens else "<unk>" for x in result]
        return result
    ...
    with open(path, 'r') as f:
        ...
        for line in f:
            ...
            for word in words:
                if word != "Ċ":
                    ids[token] = word2idx[word]
                    token += 1

    return ids

if args.test:
    test_data = batchify(load_other_test(args.test), test_batch_size, args)
...
def evaluate(data_source, batch_size=10, klass="test"):
    ...
    with open(os.path.join(args.data, f"{klass}.txt"), "r") as f:
        java_seq_lens = [len([y for y in x.strip().split() if y]) for x in f.readlines()]
        
    OUTPUT = "output_SW10_java_8b_10t_varl_mask_random05_oneturn_full_msgjava_def_test"
    ori_msgs = []
    for it in range(0, 20000, 1):
        try:
            with open(f'data/{OUTPUT}/msgs/msgs{it}.txt', 'r') as f:
                ori_msgs.append([[round(float(x)) for x in f.read().split()]])
        except:
            traceback.print_exc()
            break
    ...
    while i < data_source.size(0):
        ...
        if not args.fixed_length:
            ...
            while seq_len < 1 and code_num < len(seq_lens):
                ...
                if seq_lens[code_num] == 0 and java_seq_lens[code_num] != 0:
                    with open(f"data/decode_{args.save}{suffix}/data_out/data_out{batches_count}.txt", "w") as f:
                        f.write('a')
                    with open(f"data/decode_{args.save}{suffix}/msgs/msgs{batches_count}.txt", "w") as f:
                        f.write(' '.join(['1'] * args.msg_len))
                    with open(f"data/decode_{args.save}{suffix}/msgs_out/msgs_out{batches_count}.txt", "w") as f:
                        f.write(' '.join(['0'] * args.msg_len))
                    with open(f"data/decode_{args.save}{suffix}/msgs_out_sig/msgs_out_sig{batches_count}.txt", "w") as f:
                        f.write(' '.join(['0'] * args.msg_len))
                    batches_count += 1
                    
                code_num += 1
            if code_num >= len(seq_lens):
                break
            # if seq_len == 0:
            #     with open(f"data/decode_{args.save}/data/data{batches_count}.txt", "w") as f:
            #         f.write('')
            #     with open(f"data/decode_{args.save}/data_out/data_out{batches_count}.txt", "w") as f:
            #         f.write('')
            #     with open(f"data/decode_{args.save}/msgs/msgs{batches_count}.txt", "w") as f:
            #         f.write('0 0 0 0')
            #     with open(f"data/decode_{args.save}/msgs_out/msgs_out{batches_count}.txt", "w") as f:
            #         f.write('1 1 1 1')
            #     with open(f"data/decode_{args.save}/msgs_out_sig/msgs_out_sig{batches_count}.txt", "w") as f:
            #         f.write('1 1 1 1')
            #     continue
            ori_seq_len = seq_len
        ...
        msgs = torch.tensor(ori_msgs[batches_count]).float().cuda()
        #get a batch of fake (edited) sequence from the generator
        start_time = time.time()
        fake_data_emb = model_gen.forward_sent(data,msgs,args.gumbel_temp,only_embedding=True)
        ...
        np.savetxt(f"data/decode_{args.save}{suffix}/msgs/msgs{batches_count}.txt", msgs.detach().cpu().numpy())
        np.savetxt(f"data/decode_{args.save}{suffix}/msgs_out/msgs_out{batches_count}.txt", msg_out.detach().cpu().numpy())
        np.savetxt(f"data/decode_{args.save}{suffix}/msgs_out_sig/msgs_out_sig{batches_count}.txt", torch.nn.Sigmoid()(msg_out.detach().cpu()).numpy())
    ...
    return total_loss_reconst/batches_count, total_loss_gen / batches_count, total_loss_msg.item() / batches_count, total_loss_sem / batches_count, total_loss_lm/batches_count, total_loss_disc / batches_count
