...
class TranslatorGeneratorModel(nn.Module):
    ...
    def forward_sent_decoder(self, both_embeddings, input_data, sent_encoder_out, gumbel_temp, identifiers_mask):
        ...
        input_decoder[0:input_data_emb.size(0),:,:] = input_data_emb[0:input_data_emb.size(0),:,:]
        ...
        sent_decoded_vocab = torch.zeros(0,self.ntoken).cuda()
        sent_decoded_vocab_hot = torch.zeros(0,self.ntoken).cuda()
        if identifiers_mask == None:
            sent_decoded_vocab_mask = torch.zeros(sent_decoded.size(1),self.ntoken).cuda()
        else:
            sent_decoded_vocab_mask = identifiers_mask
        for sent_decoded_i in sent_decoded:
            sent_decoded_vocab_i = self.decoder(sent_decoded_i)
            sent_decoded_vocab = torch.cat([sent_decoded_vocab, sent_decoded_vocab_i])
            ### sent_decoded_vocab_i = self.decoder_dropout(sent_decoded_vocab_i)
            sent_decoded_vocab_i[sent_decoded_vocab_mask > 0.5] = -10.
            random_mask = torch.randn(sent_decoded.size(1),self.ntoken).cuda()
            sent_decoded_vocab_i[random_mask < 0.5] = -10.
            sent_decoded_vocab_hot_i = F.gumbel_softmax(F.log_softmax(sent_decoded_vocab_i, dim=-1), tau=gumbel_temp, hard=True)
            sent_decoded_vocab_hot = torch.cat([sent_decoded_vocab_hot, sent_decoded_vocab_hot_i])
            sent_decoded_vocab_mask[sent_decoded_vocab_hot_i > 0.5] = 1.
    ...
    def forward_sent(self, input,msg_input, gumbel_temp, only_embedding=False, identifiers_mask=None):
        if only_embedding:
        ...
        sent_decoded_vocab_emb, sent_decoded_vocab_hot, sent_decoded_vocab_soft = self.forward_sent_decoder(sent_msg_embedding, input, encoder_out, gumbel_temp, identifiers_mask)
