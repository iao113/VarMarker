...
import sys
import data
import model_mt_autoenc_cce_dropout
import lang_model
...
log_file_loss_val = open(f'log_file_loss_val_drop_{args.save}.txt','w') 
log_file_loss_train = open(f'log_file_loss_train_drop_{args.save}.txt','w') 
...
def model_load(fn):
    global model_gen, criterion, criterion_reconst, optimizer_gen # , model_disc, optimizer_disc
...
def learing_rate_scheduler():
    ...
    lr = np.power(d_model, -0.8) * min(np.power(step_num, -0.6), step_num*np.power(warm_up, -1.5))
    return lr
...
if args.resume:
    ...
    model_gen = model_mt_autoenc_cce_dropout.TranslatorGeneratorModel(ntokens, args.emsize, args.msg_len, args.msg_in_mlp_layers , args.msg_in_mlp_nodes, args.encoding_layers, args.dropout_transformer, args.dropouti, args.dropoute, args.tied, args.shared_encoder, args.attn_heads,autoenc_model)
    # model_disc = model_mt_autoenc_cce_dropout.TranslatorDiscriminatorModel(args.emsize, args.adv_encoding_layers, args.dropout_transformer, args.adv_attn_heads, args.dropouti)
...
def evaluate(data_source, batch_size=10):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        if not args.fixed_length:
            ...
            seq_len = max(1, int(np.random.normal(bptt, 5)))
        ...
        if True:
            batch_sum = fake_one_hot.sum(dim=0)
            dim_count = F.sigmoid((batch_sum - 0.5) * 30)
            dim_sum = dim_count.sum(dim=-1)
            dim_loss = -torch.log(dim_sum / fake_one_hot.size(0))
            dim_loss_sum = dim_loss.sum(dim=0) / 10
            total_loss_reconst += dim_loss_sum.data
...
def train():
	...
	#get the embeddings from the generator network of the real 
        ...
        if args.use_semantic_loss: 
            ...
            errG = args.msg_weight*errG_msg + args.sem_weight*errG_sem # + args.reconst_weight*errG_reconst args.gen_weight*errG_disc + 
            sem_losses.append(errG_sem.item())
            total_loss_sem += errG_sem.item()
        else:
            errG = args.msg_weight*errG_msg # + args.reconst_weight*errG_reconst args.gen_weight*errG_disc + 
        ...
        if False:
            ...
            dim_sum = dim_count.sum(dim=-1)
            dim_loss = -torch.log(dim_sum / fake_one_hot.size(0))
            dim_loss_sum = dim_loss.sum(dim=0) / 80
            errG = errG + args.reconst_weight*dim_loss_sum
            total_loss_reconst += dim_loss_sum.data
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | gen lr {:05.10f} | disc lr {:05.1f} | ms/batch {:3.2f} | '
                    'gen loss {:2.1f} | disc loss {:2.1f} | msg loss {:6.5f} | sem loss {:2.1f} | reconst loss {:5.5f} | lm loss {:2.1f}'.format(
                epoch, batch, len(train_data) // args.bptt, optimizer_gen.param_groups[0]['lr'], 0., # optimizer_disc.param_groups[0]['lr'],
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer_gen.param_groups[0]:
            ...
            print('-' * 89)
            log_file_loss_val.write(str(optimizer_gen.param_groups[0]['lr']) + ', ' + str(val_loss_gen2) + ', '+ str(val_loss_disc2) + ', '+ str(val_loss_msg2) + ', '+ str(val_loss_sem2) + ', '+ str(val_loss_reconst2) + ', '+ str(val_loss_lm2) + '\n')
            ...
            print('-' * 89)
            print('| end of epoch {:3d} | time: {:5.2f}s | val gen loss {:5.2f} | '
                'val disc loss {:5.2f} | val msg loss {:5.5f} | val sem loss {:5.2f} | val reconst loss {:5.2f} | val lm loss {:5.2f}'.format(
                    epoch, (time.time() - epoch_start_time), val_loss_gen, val_loss_disc, val_loss_msg, val_loss_sem, val_loss_reconst, val_loss_lm))
            print('-' * 89)
            log_file_loss_val.write(str(optimizer_gen.param_groups[0]['lr']) + ', ' + str(val_loss_gen) + ', '+ str(val_loss_disc) + ', '+ str(val_loss_msg) + ', '+ str(val_loss_sem) + ', '+ str(val_loss_reconst) + ', '+ str(val_loss_lm) + '\n')
            log_file_loss_val.flush()
